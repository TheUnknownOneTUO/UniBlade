...
let displayedStaminaUi = player.stamina
...
function updPlayerStatsUi() {
 ...
 //FOR STAMINA
    ...
    if (player.stamina < displayedStaminaUi) {

    //decrement by 10
    displayedStaminaUi -= 10

    //check if it is lower than actual stamina
    if (displayedStaminaUi < player.stamina) {

      displayedStaminaUi = player.stamina
  ...
  else if (player.stamina > displayedStaminaUi) {

    //decrement by 10
    displayedStaminaUi += 5
  ...
  if (displayedHealthUi === 0 && player.health === 0) {
    ...
    Deranged Sacrifice: ${player.kills.derangedSacrifice} <br>
    Klums: ${player.kills.klums} <br>
    Ploof: ${player.kills.ploof}
    `
...
function naturalStaminaRecovery() {
  
  //temp: hard value of 600 for now
  if (player.stamina < 600) {
...
function drawPlayerStats() {
  ...
  const actualHealthBarWidth = healthBarWidth * healthPercent
  CTX.fillStyle = 'rgba(255, 0, 0, 0.4)'
  CTX.fillRect(10, 10, actualHealthBarWidth, healthBarHeight)
  
  //FOR STAMINA
  
  const staminaBarWidth = CANVAS_WIDTH * 0.3
  const staminaBarHeight = 15

  CTX.fillStyle = 'rgba(255, 255, 255, 0.4)'
  CTX.fillRect(10, 50, staminaBarWidth, staminaBarHeight)

  //temp: 600 is player max stamina
  const staminaPercent = displayedStaminaUi / 600

  const actualStaminaBarWidth = staminaBarWidth * staminaPercent
  CTX.fillStyle = player.stamina < 200 ? 'rgba(100, 100, 255, 0.4)' : 'rgba(0, 0, 255, 0.4)';

  CTX.fillRect(10, 50, actualStaminaBarWidth, staminaBarHeight)
...
class ExplosionFx extends Particles {
  constructor() {

    super('Explosion Fx', 'one-shot', 64, 64, null, explosionFxSprite)
    this.animationDone = false
    this.maxFrame = 16
    this.isStatic = true
...
class FireballDeflectFx extends Particles {
  constructor() {

    super('Fireball Deflect Fx', 'one-shot', 16, 16, null, fireballDeflectFxSprite)
    this.animationDone = false
    this.maxFrame = 4
    this.isStatic = true
...
const explosionFxSprite = new Image()
explosionFxSprite.src = 'assets/particlesSprites/explosion.png'

const fireballDeflectFxSprite = new Image() 
fireballDeflectFxSprite.src = 'assets/particlesSprites/fireball_deflected_vfx.png'
...
function spawnParticles(name, i, xPos, yPos) {
  ...
  else if (name === 'Explosion Fx') {

    const newParticles = new ExplosionFx()
    newParticles.xPos = xPos - 32
    newParticles.yPos = yPos - 32

    oneShotParticlesArray.push(newParticles)

  }
  
  else if (name === 'Fireball Deflect Fx') {

    const newParticles = new FireballDeflectFx()
    newParticles.xPos = xPos
    newParticles.yPos = yPos
...
function checkMeleeHit() {
  ...
  removeDeadEnemies()
  checkMeleeHitForProjectiles()

}

function checkMeleeHitForProjectiles() {

  projectileArray.forEach((proj, i) => {

    if (proj.deflectionType === 'destroy') {

      //no need to get trueXY because it is what moved projectiles instead of xyPos

      const dist = calculateDistance(meleeCenterAttackX, meleeCenterAttackY, proj.trueX, proj.trueY)

      //make player the origin
      //y needs to be inverted
      const relX = proj.trueX - player.centerX;
      const relY = (proj.trueY - player.centerY) * -1;

      const angle = calculateAngle(relX, relY);

      //temp 32 is blade radius and width for radius for nos
      //can't use meleeStartAngle and MeleeEndAngle because it can go over 360° which messes up comparation
      if (dist - 32 - proj.width <= 0 && angle >= meleeCenterAngle - 90 && angle <= meleeCenterAngle + 90) {

        proj.hasDeflected = true
  ...
  removeDeflectedProjectiles()

}

function removeDeflectedProjectiles() {

  for (let i = projectileArray.length - 1; i >= 0; i--) {

    const proj = projectileArray[i];

    if (proj.hasDeflected !== null) {

      if (proj.hasDeflected === true) {
        
        //upd cornerXY
        proj.cornerX = proj.trueX - proj.width
        proj.cornerY = proj.trueY - proj.height
        
        spawnParticles('Fireball Deflect Fx', null, proj.cornerX, proj.cornerY)

        const removedProj = projectileArray.splice(i, 1)
...
function checkRangedHit(i) {
  ...
  removeDeadEnemies()
  checkRangedHitForProjectiles(i)

}

function checkRangedHitForProjectiles(i) {
  ...
  projectileArray.forEach((proj, i) => {
    ...
    if (proj.hasDeflected !== null) {
      ...
      if (proj.deflectionType === 'destroy') {
        ...
        const dist = calculateDistance(ranged.trueX, ranged.trueY, proj.trueX, proj.trueY)

        if (dist <= ranged.width + proj.width) {
  ...
  for (let i = enemiesArray.length - 1; i >= 0; i--) {
    ...
    if (enemy.health <= 0) {
      ...
      if (enemy.health === -5000) {
        //-5000 is self detonation
        spawnParticles('Explosion Fx', null, enemy.xPos, enemy.yPos)
      ...
      } else if (killedEnemy[0].name === ' Klums') {

        player.kills.klums++
        player.score += 7

      } else if (killedEnemy[0].name === 'Ploof') {
...
const klumsSprite = new Image()
klumsSprite.src = 'assets/characterSprites/klums.png'

const ploofSprite = new Image()
ploofSprite.src = 'assets/characterSprites/ploof.png'

class Neophyte extends Enemy {
  constructor() {

    super('Neophyte', 1000, Math.random() + 0.3, 16, 16, 1, 'inPursue', neophyteSprite)
...
class SpellCaster extends Enemy {
  constructor() {
    ...
    super('Spell Caster', 1500, 1, 16, 16, 1, 'keepingDistance', spellCasterSprite)
...
class DerangedSacrifice extends Enemy {
  constructor() {
    ...
    super('Deranged Sacrifice', 1800, Math.random() + 0.8, 16, 16, 1, 'inPursue', derangedSacrificeSprite)
...
class Klums extends Enemy {
  constructor() {

    super('Klums', 1200, 0.7, 16, 16, 0.2, 'inPursue', klumsSprite)

    this.state = 'wandering'
    this.desiredPosition = { x: null, y: null }
    this.detonationCounter = 160
...
class Ploof extends Enemy {
  constructor() {

    super('Ploof', 1700, 0.7, 16, 16, 0.4, 'inPursue', ploofSprite)
...
function spawnEnemies() {
  ...
  for (let i = 1; i <= numberOfEnemies; i++) {
    ...
    const array = [Neophyte, SpellCaster, DerangedSacrifice, Klums, Ploof]
...
class Projectile {
  constructor(type, maxSpeed, width, height, damage, deflectionType, sprite) {
...
const fireballProjectileSprite = new Image()
fireballProjectileSprite.src = 'assets/projectileSprites/fireballProjectile.png'

class Orb extends Projectile {
  constructor() {

    super('Orb', 2, 8, 8, 50, 'ignore', orbProjectileSprite)
...
function spawnProjectile(type, velX, velY, trueX, trueY, launchedAngle) {
  ...
  if (type === 'Orb') {
    ...
    newProjectile.launchedAngle = launchedAngle;
    ...
    const newProjectile = new Fireball();
    newProjectile.velX = velX;
    newProjectile.velY = velY;
    newProjectile.trueX = trueX;
    newProjectile.trueY = trueY;
...
function drawProjectiles() {
  ...
  projectileArray.forEach((proj, i) => {
    ...
    if (proj.launchedAngle === null) {

      CTX.drawImage(
        //spritesheet
        proj.sprite,
        //sx
        (proj.xFrame * proj.width) + proj.xFrame,
        //sy
        (proj.yFrame * proj.height) + proj.yFrame,
        //sWidth
        proj.width,
        //sHeight
        proj.height,
        //canvas x
        proj.trueX - proj.width,
        //canvas y
        proj.trueY - proj.height,
        //scale x
        proj.width * 2,
        //scale y
        proj.height * 2
    ...
    else if (proj.launchedAngle !== null) {

      //convert calculated angle to radians

      let shiftedAngle = proj.launchedAngle - 90
      if (shiftedAngle < 0) {
        shiftedAngle = 360 + shiftedAngle
      }
      ...
      CTX.translate(proj.trueX, proj.trueY)
...
function enemyStateHandler() {
  ...
  enemiesArray.forEach((enemy, i) => {
    ...
    if (enemy.state === 'inPursue') {
      ...
      if (enemy.desiredPosition.x === null && enemy.desiredPosition.y === null) {
        ...
        if (distToDesired <= 1) {
          ...
          //to determine what state after wandering
          afterWandering(i)
    ...
    else if (enemy.state === 'gettingUp') {

      //nothing, prev state handles everything 
      ...
      if (enemy.isFleeing === false) {
        enemy.isFleeing = true
        enemy.maxSpeed = 3

        const randomVel = Math.random()
        const dominantVelAxis = Math.round(Math.random() * 100)
        let isXnegative = Math.round(Math.random() * 100)
        let isYnegative = Math.round(Math.random() * 100)
        ...
        if (dominantVelAxis <= 50) {

          enemy.xVel = 1 * isXnegative
          enemy.yVel = randomVel * isYnegative

        }

        else {

          enemy.xVel = randomVel * isXnegative
          enemy.yVel = 1 * isYnegative
...
function afterWandering(i) {

  let enemy = enemiesArray[i]

  if (enemy.name === 'Klums') {

    let decisionRng = Math.round(Math.random() * 100)
...
function orbProjectileCasting(i) {
  ...
  function launchProjectile() {
    ...
    if (launchCount < 3) {
      ...
      spawnProjectile('Orb', projVel.calcX, projVel.calcY, enemy.trueX, enemy.trueY, null)
...
function fireballProjectileFiring(i) {
  ...
  function launchProjectile() {
    ...
    if (launchCount < 5) {
      ...
      //calculate the angle
      let angleToPlayer = calculateAngle(playerRelX, playerRelY);
      ...
      spawnProjectile('Fireball', projVel.calcX, projVel.calcY, enemy.trueX, enemy.trueY, angleToPlayer)
...
function pursue(i) {
  ...
  if (enemy.name === 'Neophyte') {
    ...
    panicCharge(i, doesVectorCol)

  }

  else if (enemy.name === 'Ploof') {

    closeIn(i, doesVectorCol)
...
function panicCharge(i, doesVectorCol) {
  ...
  const distToPlayer = calculateDistance(enemy.trueX, enemy.trueY, player.centerX, player.centerY)

  //check if distance is close enough (may change)
  //starts detonation
  if (distToPlayer <= 160 && enemy.detonationCounter === 160) {
    ...
    enemy.invulnerable = true
    enemy.detonationCounter--

    return

  }

  if (enemy.detonationCounter < 160 && enemy.detonationCounter > 0) {
  ...
  if (enemy.detonationCounter === 0) {

    //32 is the blast radius
    if (distToPlayer <= player.width + 32) {
    ...
    //-5000 to differentiate between killed or detonating
    enemy.health = -5000
    removeDeadEnemies()
  ...
  let tripPercent = Math.round(Math.random() * 1000)
  //based on rng and whether it is currently volliding with player
  if (tripPercent <= 3 && distToPlayer > enemy.width + player.width) {

    enemy.detonationCounter = 160
    enemy.state = 'tripped'
    enemy.invulnerable = false
    return
  ...
  if (!doesVectorCol && distToPlayer > enemy.width + player.width) {
...
function closeIn(i, doesVectorCol) {
  ...
  let randomRange = Math.round(Math.random() * 30)

  if (randomRange + distToPlayer <= 200) {
...
function wandering(i) {

  const enemy = enemiesArray[i]

  enemy.desiredPosition.x = Math.round(Math.random() * CANVAS_WIDTH)
  enemy.desiredPosition.y = Math.round(Math.random() * CANVAS_HEIGHT)
...
function drawEnemies() {
  ...
  enemiesArray.forEach((enemy, i) => {
    ...
    else if (enemy.state === 'inPursue' || enemy.state === 'keepingDistance' || enemy.state === 'wandering' || enemy.state === 'fleeing' || enemy.state === 'firing') {

      if (enemy.name === 'Klums' && enemy.detonationCounter < 160) {
    ...
    } else if (enemy.state === 'detonating') {
...
function fireballProjectileAnim(i) {
